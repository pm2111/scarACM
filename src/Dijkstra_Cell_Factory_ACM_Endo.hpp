...
public:
    DijkstraStimuliCellFactory(ORdGksVarierDTI024* pGksVarier,
                              ...
                              const std::vector<unsigned>& rLvRootNodes,
                              const std::vector<unsigned>& rRvRootNodes,
                              double speed,double period,unsigned num_nodes, const std::vector<std::vector<double> >& regions,const std::map<unsigned,unsigned>& MapNewOldIndex,std::string edge_nodes_path,std::string mindists_path)
...
*/
	...
	double conductivity_scaling_factor =mregions[mMapNewOldIndex.at(node_index)][0];
	...
	/*if (conductivity_scaling_factor < 1)  //nested loop, assign ACM cell model only for the nodes with fibrosis
	{
